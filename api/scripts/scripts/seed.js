...
const Company_1 = require("../src/models/Company");
const Contact_1 = require("../src/models/Contact");
const Deal_1 = require("../src/models/Deal");
...
class DatabaseSeeder {
    ...
    async connect() {
        try {
            await (0, mongoose_1.connect)(environment_1.env.MONGODB_URI);
    ...
    async seedUsers() {
        ...
        for (let i = 0; i < ORGANIZATIONS.length; i++) {
            ...
            const adminUser = await User_1.User.create({
                ...
                permissions: [common_1.Permission.USER_READ, common_1.Permission.USER_CREATE, common_1.Permission.USER_UPDATE, common_1.Permission.USER_DELETE, common_1.Permission.DEAL_READ, common_1.Permission.DEAL_CREATE, common_1.Permission.CONTACT_READ, common_1.Permission.CONTACT_CREATE],
            ...
            for (let j = 0; j < 2; j++) {
                const managerUser = await User_1.User.create({
                    ...
                    password: hashedPassword,
                    role: common_1.UserRole.MANAGER,
                    permissions: [common_1.Permission.DEAL_READ, common_1.Permission.DEAL_CREATE, common_1.Permission.DEAL_UPDATE, common_1.Permission.CONTACT_READ, common_1.Permission.CONTACT_CREATE],
                    isEmailVerified: true,
            ...
            // Create sales reps
            for (let k = 0; k < 3; k++) {
                const salesUser = await User_1.User.create({
                    firstName: FIRST_NAMES[Math.floor(Math.random() * FIRST_NAMES.length)],
                    lastName: LAST_NAMES[Math.floor(Math.random() * LAST_NAMES.length)],
                    email: `sales${k + 1}@${org.domain}`,
                    password: hashedPassword,
                    role: common_1.UserRole.SALES_REP,
                    permissions: [common_1.Permission.DEAL_READ, common_1.Permission.DEAL_CREATE, common_1.Permission.CONTACT_READ, common_1.Permission.CONTACT_CREATE],
                    isEmailVerified: true,
                    organizationId: adminUser._id.toString(),
                    organization: {
                        name: org.name,
    ...
    async seedPipelines() {
        ...
        for (let i = 0; i < this.organizationIds.length; i++) {
            ...
            // Sales Pipeline
            const salesPipeline = await Pipeline_1.Pipeline.create({
                name: 'Sales Pipeline',
            ...
            const successPipeline = await Pipeline_1.Pipeline.create({
        ...
        for (let i = 0; i < this.companyIds.length; i++) {
            ...
            for (let j = 0; j < contactCount; j++) {
                ...
                const contact = await Contact_1.Contact.create({
                    ...
                    type: j === 0 ? common_1.ContactType.CUSTOMER : [common_1.ContactType.LEAD, common_1.ContactType.PROSPECT, common_1.ContactType.CUSTOMER][Math.floor(Math.random() * 3)],
                    companyId,
                    organizationId: orgId,
                    createdBy: orgId,
    ...
    async seedDeals() {
        ...
        for (let i = 0; i < dealCount; i++) {
            ...
            const pipeline = await Pipeline_1.Pipeline.findById(pipelineId);
            ...
            const deal = await Deal_1.Deal.create({
                ...
                value: dealValue,
                currency: 'USD',
                priority: [common_1.DealPriority.LOW, common_1.DealPriority.MEDIUM, common_1.DealPriority.HIGH, common_1.DealPriority.URGENT][Math.floor(Math.random() * 4)],
                status: randomStage?.name === 'Closed Won' ? common_1.DealStatus.WON :
                    randomStage?.name === 'Closed Lost' ? common_1.DealStatus.LOST : common_1.DealStatus.OPEN,
    ...
    async seedActivities() {
        ...
        for (let i = 0; i < this.dealIds.length; i++) {
            ...
            const pastActivity = await Activity_1.Activity.create({
                title: 'Discovery Call',
                description: 'Initial discovery call to understand requirements and budget',
                type: common_1.ActivityType.CALL,
                status: common_1.ActivityStatus.COMPLETED,
                priority: common_1.DealPriority.MEDIUM,
            ...
            const futureActivity = await Activity_1.Activity.create({
                title: 'Follow-up Meeting',
                description: 'Follow-up meeting to present proposal and discuss next steps',
                type: [common_1.ActivityType.MEETING, common_1.ActivityType.CALL, common_1.ActivityType.EMAIL][Math.floor(Math.random() * 3)],
                status: common_1.ActivityStatus.PENDING,
                priority: [common_1.DealPriority.LOW, common_1.DealPriority.MEDIUM, common_1.DealPriority.HIGH][Math.floor(Math.random() * 3)],
                dueDate: new Date(Date.now() + Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000),
                dealId,
                contactId,
                userId,
