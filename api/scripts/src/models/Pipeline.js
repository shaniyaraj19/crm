...
pipelineSchema.virtual('closedWonStages').get(function () {
    return this.stages.filter(stage => stage.isClosedWon);
});
// Virtual for closed lost stages
pipelineSchema.virtual('closedLostStages').get(function () {
...
pipelineSchema.methods.getStageById = function (stageId) {
    return this.stages.find((stage) => stage._id?.toString() === stageId);
};
...
pipelineSchema.methods.getNextStage = function (currentStageId) {
    ...
    if (!currentStage)
        return undefined;
    const nextStage = this.stages.find((stage) => stage.order === currentStage.order + 1 && stage.isActive);
    return nextStage;
};
...
pipelineSchema.methods.getPreviousStage = function (currentStageId) {
    ...
    if (!currentStage)
        return undefined;
    const previousStage = this.stages.find((stage) => stage.order === currentStage.order - 1 && stage.isActive);
    return previousStage;
};
// Method to validate stage transition
pipelineSchema.methods.canTransitionToStage = function (fromStageId, toStageId) {
